...
function formatRelative(iso) {
    ...
    if (minSince < 1)
        return { text: "Synced just now", tone: "green" };
    if (minSince < 60)
        return { text: `Synced ${minSince} min ago`, tone: "green" };
    const hSince = Math.round(minSince / 60);
    if (hSince < 24)
        return { text: `Synced ${hSince}h ago`, tone: "muted" };
    const daysSince = Math.round(hSince / 24);
    if (daysSince === 1)
        return { text: "Synced yesterday", tone: "muted" };
    if (daysSince <= 7)
        return { text: `Synced ${daysSince}d ago`, tone: "amber" };
    return { text: `Stale — last synced ${daysSince}d ago`, tone: "red" };
}
export default function SyncFreshnessChip({ syncedAt, label, compact = false, }) {
    ...
    if (!syncedAt) {
        return (_jsxs("span", { className: `inline-flex items-center gap-1 ${compact ? "text-[10px]" : "text-[11px]"} text-text-soft`, title: "No sync recorded yet", children: [_jsx("span", { "aria-hidden": "true", children: "\u25CB" }), "Never synced"] }));
    }
    ...
    return (_jsxs("span", { className: `inline-flex items-center gap-1 ${compact ? "text-[10px]" : "text-[11px]"} ${toneClass} tabular-nums`, title: `Last sync: ${new Date(syncedAt).toLocaleString()}`, "aria-label": `${labelText}. Last sync at ${new Date(syncedAt).toLocaleString()}.`, children: [_jsx("span", { className: dotColor, "aria-hidden": "true", children: dotChar }), labelText] }));
}
