  ...
  return (
    <div>
      ...
      <div className="bg-card border border-border rounded-md shadow-card p-5 mb-6">
        ...
        {/* OLD HEADLINE BLOCK REMOVED — replaced by BudgetHero + StatStrip
            above. The aggregate progress bar moved INTO the hero. The
            ternary closes here with the original `)}`. */}
        {false && (
          <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-5">
            {/* Sprint H-1a — INCOME card, leftmost. Surfaces 3-mo rolling
                avg inflows so the user can compare planned-spending vs
                actual take-home at a glance. */}
            <div>
              <div className="text-xs text-text-muted uppercase tracking-wide">
                Income
              </div>
              <div
                className="text-2xl font-semibold tabular-nums mt-1 text-inflow"
                title="3-month rolling average of all positive inflows — includes Livio payroll AND outliers (settlement payouts, Venmo, Dave). Use this to compare to actual spending."
              >
                {fmtCents(data?.monthly_income_cents ?? 0)}
              </div>
              <div className="text-[10px] text-text-soft mt-0.5">
                3-mo avg
                {data?.recurring_income_cents != null &&
                  data.recurring_income_cents > 0 &&
                  data.recurring_income_cents !== data.monthly_income_cents && (
                    <span
                      className="ml-1"
                      title="Recurring payroll only (Livio wires). This is the conservative number used for future projections — outliers don't count as expected income."
                    >
                      · {fmtCents(data.recurring_income_cents)} recurring
                    </span>
                  )}
              </div>
            </div>
            <div>
              ...
              <div
                className="text-2xl font-semibold tabular-nums mt-1"
                title={
                  data?.real_budget_cents != null
                    ? `Real budget excludes catch-all caps (Transfer, Uncategorized, etc.). Sum of all caps is ${fmtCents(data.total_budget_cents)}.`
                    : undefined
                }
              >
                {fmtCents(data?.real_budget_cents ?? data?.total_budget_cents ?? 0)}
              </div>
              {data?.real_budget_cents != null &&
                data.real_budget_cents !== data.total_budget_cents && (
                  <div className="text-[10px] text-text-soft mt-0.5">
                    real cap (raw {fmtCents(data.total_budget_cents)})
                  </div>
                )}
            </div>
            <div>
              ...
              <div className="text-2xl font-semibold tabular-nums mt-1 text-outflow">
                {fmtCents(data?.total_actual_cents ?? 0)}
              </div>
            </div>
            <div>
              <div className="text-xs text-text-muted uppercase tracking-wide">
              ...
              <div
                className={`text-2xl font-semibold tabular-nums mt-1 ${
                  (data?.real_budget_cents ?? data?.total_budget_cents ?? 0) -
                    (data?.total_actual_cents ?? 0) >=
                  0
                    ? "text-inflow"
                    : "text-outflow"
                }`}
            ...
            <div>
              ...
              <div className="text-2xl font-semibold tabular-nums mt-1 text-text-muted">
                {fmtCents(
                  unbudgeted.reduce((acc, u) => acc + u.actual_outflow_cents, 0)
                )}
              </div>
